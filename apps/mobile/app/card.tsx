...
export default function CardScreen() {
  const storedCardId = useAppStore((state) => state.cardId);
  const setCardId = useAppStore((state) => state.setCardId);
  const persistedCard = useQuery({ queryKey: ["current-card"], queryFn: api.currentCard });
  useEffect(() => {
    const cardId = persistedCard.data?.card?.bridgeCardAccountId;
    if (cardId) setCardId(cardId);
  }, [persistedCard.data?.card?.bridgeCardAccountId, setCardId]);
  const cardId = persistedCard.data?.card?.bridgeCardAccountId ?? storedCardId;
