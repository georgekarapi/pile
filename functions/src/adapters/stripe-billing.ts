...
export async function createBillingSubscription(input: { userId: string; planId: string; amountUsd: number; priceId?: string; stripeCustomerId?: string; email?: string; idempotencyKey: string }): Promise<SubscriptionCheckout> {
  ...
  if (!input.priceId && config.PILEUP_STRIPE_PRODUCT_ID.includes("CONFIGURE_")) throw new Error("PILEUP_STRIPE_PRODUCT_ID is required for live Billing");
  const subscription = await stripe.subscriptions.create({
    customer: customerId,
    items: [input.priceId ? { price: input.priceId } : { price_data: { currency: "usd", product: config.PILEUP_STRIPE_PRODUCT_ID, recurring: { interval: "week" }, unit_amount: input.amountUsd * 100 } }],
...
export async function getBillingSubscriptionCheckout(input: { subscriptionId: string; userId: string; planId: string }): Promise<SubscriptionCheckout> {
  if (config.PILEUP_MODE === "demo") return { subscriptionId: input.subscriptionId, customerId: `demo_customer_${input.userId}`, mode: "demo" };
  if (!config.STRIPE_SECRET_KEY) throw new Error("STRIPE_SECRET_KEY is required for live Billing");
  const stripe = new Stripe(config.STRIPE_SECRET_KEY);
  ...
  if (subscription.metadata.pileupUserId !== input.userId || subscription.metadata.pileupPlanId !== input.planId) throw new Error("Stripe subscription does not match this plan");
...
export async function createPaymentMethodPortalSession(input: { subscriptionId: string; userId: string; planId: string; customerId: string }): Promise<{ url: string; defaultMethodId: string }> {
  if (config.PILEUP_MODE !== "live" || !config.STRIPE_SECRET_KEY) throw new Error("Stripe Billing is unavailable");
  const stripe = new Stripe(config.STRIPE_SECRET_KEY);
  ...
  if (subscription.metadata.pileupUserId !== input.userId || subscription.metadata.pileupPlanId !== input.planId || subscriptionCustomerId !== input.customerId) throw new Error("Stripe subscription does not match this account");
  ...
  const session = await stripe.billingPortal.sessions.create({
    ...
    ...(config.PILEUP_STRIPE_PORTAL_CONFIGURATION_ID ? { configuration: config.PILEUP_STRIPE_PORTAL_CONFIGURATION_ID } : {}),
...
export async function syncBillingPaymentMethod(input: { subscriptionId: string; userId: string; planId: string; customerId: string; previousDefaultMethodId: string }): Promise<boolean> {
  ...
  await stripe.subscriptions.update(input.subscriptionId, { default_payment_method: paymentMethodId }, { idempotencyKey: `pileup-payment-method-${input.subscriptionId}-${paymentMethodId}` });
  return true;
}
...
export async function changeBillingSubscription(input: { subscriptionId: string; userId: string; planId: string; amountUsd: number; previousPriceId?: string; idempotencyKey: string }): Promise<{ priceId: string; previousPriceId: string }> {
  if (config.PILEUP_MODE === "demo") return { priceId: `demo_price_${input.amountUsd}_${input.idempotencyKey}`, previousPriceId: input.previousPriceId ?? `demo_price_${input.planId}_legacy` };
  if (!config.STRIPE_SECRET_KEY || config.PILEUP_STRIPE_PRODUCT_ID.includes("CONFIGURE_")) throw new Error("Stripe Billing is not configured");
  ...
  if (current.metadata.pileupUserId !== input.userId || current.metadata.pileupPlanId !== input.planId || current.status !== "active") throw new Error("The active Stripe subscription does not match this plan");
  if (current.items.data.length !== 1) throw new Error("Expected exactly one subscription item");
  const item = current.items.data[0];
  const targetPrice = await stripe.prices.create({ currency: "usd", product: config.PILEUP_STRIPE_PRODUCT_ID, recurring: { interval: "week" }, unit_amount: input.amountUsd * 100, metadata: { pileupPlanId: input.planId, pileupPlanChangeKey: input.idempotencyKey } }, { idempotencyKey: `pileup-change-price-${input.idempotencyKey}` });
  ...
  }, { idempotencyKey: `pileup-change-${input.idempotencyKey}` });
...
export async function pauseBillingSubscription(input: { subscriptionId: string; userId: string; planId: string; priceId?: string; idempotencyKey: string }): Promise<void> {
  ...
  const stripe = new Stripe(config.STRIPE_SECRET_KEY);
  const subscription = await stripe.subscriptions.retrieve(input.subscriptionId);
  if (subscription.metadata.pileupUserId !== input.userId || subscription.metadata.pileupPlanId !== input.planId || subscription.status !== "active") throw new Error("Stripe subscription does not match this plan");
  ...
  await stripe.subscriptions.update(input.subscriptionId, { pause_collection: { behavior: "void" } }, { idempotencyKey: `pileup-pause-${input.idempotencyKey}` });
}
...
export async function resumeBillingSubscription(input: { subscriptionId: string; userId: string; planId: string; priceId?: string; idempotencyKey: string }): Promise<void> {
  ...
  await stripe.subscriptions.update(input.subscriptionId, { pause_collection: "" }, { idempotencyKey: `pileup-resume-${input.idempotencyKey}` });
}
