...
2. Only a verified, deduplicated webhook may create a funding cycle. A client cannot call a “buy” endpoint with an arbitrary fiat amount.
3. Each cycle progresses in order: credit → swap → deposit → borrow. Persist the state after every durable step and resume from that state; never retry an earlier movement just because a later movement failed.
...
5. All Solana transactions are built by a server-side adapter and signed by the user’s scoped Privy signer. Before signing, validate owner, permitted program IDs, token mints, maximum input, and recipients. Borrow proceeds may only go to the owner’s USDC account.
